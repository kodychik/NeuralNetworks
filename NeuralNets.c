...
int train_1layer_net(double sample[INPUTS], int label, double (*sigmoid)(double input), double weights_io[INPUTS][OUTPUTS])
{
   ...
   *
   */
  int max_class = classify_1layer(sample, label, sigmoid, weights_io);
  ...
  return max_class; // <--- This should return the class for this sample
}
...
int classify_1layer(double sample[INPUTS], int label, double (*sigmoid)(double input), double weights_io[INPUTS][OUTPUTS])
{
  ...
  {
    if (activations[j] > activations[max_class])
    {
      max_class = j;
    }
  }
...
void feedforward_1layer(double sample[785], double (*sigmoid)(double input), double weights_io[INPUTS][OUTPUTS], double activations[OUTPUTS])
{
  ...
  for (j = 0; j < OUTPUTS; j++)
  {
    for (i = 0; i < INPUTS; i++)
    {
      activations[j] += (sample[i] * weights_io[i][j]);
    }
    activations[j] = sigmoid(activations[j] * SIGMOID_SCALE);
  }
...
void backprop_1layer(double sample[INPUTS], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_io[INPUTS][OUTPUTS])
{
  ...
  for (i = 0; i < OUTPUTS; i++)
  {
    ...
    for (j = 0; j < INPUTS; j++)
    {
      change = sample[j] * activations[i] * (1 - activations[i]) * (target - activations[i]);
      weights_io[j][i] += ALPHA * change;
    }
  }
...
double logistic(double input)
{
  ...
  // TO DO: Implement this function!
  double result = 1 / (1 + exp(-1 * input));

  return result; // <--- Should return the value of the logistic function on the input
}
